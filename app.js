document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- Clinical definitions & modules (ALL) ---------- */
  const clinical = {
  ...
  // ILA => Resolution Specialist, and full module list with default distributions
  const modules = [
  ...
  function params() {
    return {
      ...
      t:{ blue:+$('tBlue').value, green:+$('tGreen').value, amber:+$('tAmber').value, red:+$('tRed').value },
  ...
  function renderWorkflow(model){
    ...
    $('workflow').innerHTML = `<div class="flex justify-center items-center gap-3 flex-wrap">${html}</div>`;
  }
  ...
  function renderModules(){
    ...
    modules.forEach((m, i)=>{
      ...
      card.innerHTML = `
        <div class="mod-header">
          ...
          <button class="btn-ghost exp" data-i="${i}"><i class="fa-solid fa-chevron-down"></i></button>
        </div>
        ...
        <div class="mod-details">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div class="p-3 rounded-lg bg-red-50 border border-red-200">
              ...
              <p class="text-xs text-gray-700"><b>Definition:</b> ${defs.red.def}</p>
    ...
    root.querySelectorAll('.mod-toggle').forEach(cb=>{
      cb.addEventListener('change', e=>{
        const i = +e.target.dataset.i; modules[i].on = e.target.checked; calc(); updateModuleStats();
      });
    });
    root.querySelectorAll('.mod-distro').forEach(inp=>{
      inp.addEventListener('input', e=>{
        const i = +e.target.dataset.i, k = e.target.dataset.k;
        modules[i].distro[k] = +e.target.value; calc(); updateModuleStats();
      });
    });
    ...
    $('enableAll').onclick = ()=>{ modules.forEach(m=>m.on=true); renderModules(); calc(); updateModuleStats(); };
    $('disableAll').onclick= ()=>{ modules.forEach(m=>m.on=false); renderModules(); calc(); updateModuleStats(); };
  }
  ...
  function calc(){
    ...
    const rows = active.map(m=>{
      const red = m.distro.red*scale, amber=m.distro.amber*scale, green=m.distro.green*scale;
    ...
    const eff = Math.max(0,1-p.thynkEff);
    const thynkFteSpec = (minutesByRole.Specialist*eff)/avail;
    const thynkFteNav  = (minutesByRole.Navigator*eff)/avail;
    const thynkCost = (thynkFteSpec*p.rateThynkSpec*1920)+(thynkFteNav*p.rateThynkNav*1920);

    // Allocate costs to modules proportionally by minutes
    rows.forEach(r=>{
    ...
    // Per-module minutes & FTE share (for details blocks)
    rows.forEach((r, i)=>{
      const share = totalMinutes? r.minutes/totalMinutes : 0;
      const specShare = minutesByRole.Specialist? Math.min(1, (r.m.primaryRole==='Specialist'? r.minutes:0)/minutesByRole.Specialist):0;
      const navShare  = minutesByRole.Navigator?  Math.min(1, (r.m.primaryRole==='Navigator'? r.minutes:0)/minutesByRole.Navigator):0;

      const html = `
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div><span class="hint">Module minutes</span><div class="font-semibold">${fmt0(r.minutes)}</div></div>
          <div><span class="hint">Total minutes</span><div class="font-semibold">${fmt0(totalMinutes)}</div></div>
          <div><span class="hint">FTE share (module/total)</span><div class="font-semibold">${(share*100).toFixed(1)}%</div></div>
          <div><span class="hint">${r.m.primaryRole==='Navigator'?'Navigator':'Specialist'} minutes</span><div class="font-semibold">${fmt0(r.minutes)}</div></div>
        </div>`;
      const node = $(`mstats-${i}`);
  ...
  function updateModuleStats(){ /* filled inside calc via mstats- blocks */ }

  // Render refs
  $('refs').innerHTML = references.map(r=>`<li>[${r.id}] ${r.txt}</li>`).join('');
